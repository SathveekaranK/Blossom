    ...
    return (
        <div className="flex flex-col gap-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 bg-white border border-primary/10 px-5 py-2.5 rounded-2xl shadow-sm">
                    <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-dark">
                            ...
                            {isLoading ? (
                                ...
                                filteredUsers.map((user) => (
                                    <React.Fragment key={user.id}>
                                        <tr className={`transition-colors group ${expandedUser === user.id ? 'bg-white/50' : 'hover:bg-primary/50'}`}>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-4">
                                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-heading font-bold text-xl flex-shrink-0 shadow-sm border ${user.role === 'ADMIN' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-white text-dark border-primary/10'}`}>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border w-fit ${user.role === 'ADMIN'
                                                    ...
                                                    : 'bg-light text-dark border-primary/10'
                                                    }`}>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-8 h-8 rounded-full bg-light flex items-center justify-center text-dark">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <button
                                                    ...
                                                    className={`p-2 rounded-xl transition-all inline-flex items-center justify-center border ${expandedUser === user.id ? 'bg-dark text-white border-dark shadow-md' : 'bg-white text-dark border-primary/10 hover:bg-white'}`}
                                                    title="View Details"
                                                >
                                                ...
                                                <tr key={`${user.id}-details`}>
                                                    <td colSpan={6} className="p-0 border-b border-primary/10">
                                                        <motion.div
                                                            ...
                                                            className="overflow-hidden bg-white border-x-4 border-l-dark border-r-transparent"
                                                        >
                                                                        ...
                                                                        {userDetails.orders && userDetails.orders.length > 0 ? (
                                                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                                                {userDetails.orders.map((order: any) => (
                                                                                    <div key={order.id} className="p-5 bg-white rounded-2xl border border-primary/10 shadow-sm flex flex-col gap-4">
                                                                                        <div className="flex items-center justify-between">
                                                                                        ...
                                                                                        <div className="flex items-center justify-between">
                                                                                            <span className="text-xs font-medium bg-light px-2 py-1 rounded-md text-muted">
                                                                                                {order.items.length} items
                                                                                            </span>
                                                                                            <span className="text-base font-bold text-dark">₹{Number(order.totalAmount).toFixed(2)}</span>
                                                                                        </div>
                                                                                        <span className="text-xs text-muted font-medium flex items-center gap-1.5 pt-4 border-t border-primary/10">
                                                                            ...
                                                                            <p className="text-sm font-medium text-muted bg-white p-6 rounded-2xl border border-primary/10 text-center">No order history available.</p>
                                                                        )}
                                                                    </div>
                                                                ) : null}
                                                            </div>
